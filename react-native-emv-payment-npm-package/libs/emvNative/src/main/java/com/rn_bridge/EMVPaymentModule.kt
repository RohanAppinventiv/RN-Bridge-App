...
class EMVPaymentModule(reactContext: ReactApplicationContext) :
                ...
                currentActivity
                        ?: run {
                            throw UninitializedPropertyAccessException(
                                    "EMVPaymentModule: React activity hasn't been initialised "
                            )
                            return
                        }
    ...
    fun startEMVTransaction(amount: Double) {
        CoroutineScope(Dispatchers.Main).launch { emvBridge.startEMVSaleTransaction(amount) }
    }

    @ReactMethod
    fun getCardDetails() {
        CoroutineScope(Dispatchers.Main).launch { emvBridge.collectEMVCardData() }
    }

    @ReactMethod
    fun cancelTransaction() {
        CoroutineScope(Dispatchers.Main).launch { emvBridge.clearAllListeners() }
    }

    @ReactMethod
    fun printReceipt(str: String) {
        CoroutineScope(Dispatchers.Main).launch { emvBridge.printReceipt(str) }
    }
