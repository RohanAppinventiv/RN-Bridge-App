...
class DsiEMVManagerModule(private val reactContext: ReactApplicationContext) :
    ...
    fun runSaleTransaction(amount: String) {
        CoroutineScope(Dispatchers.Main).launch { dsiEMVManager?.runSaleTransaction(amount) }
    }

    @ReactMethod
    fun collectCardDetails() {
        CoroutineScope(Dispatchers.Main).launch { dsiEMVManager?.collectCardDetails() }
    }

    @ReactMethod
    fun setupConfig() {
        CoroutineScope(Dispatchers.Main).launch { dsiEMVManager?.checkConfig() }
    }

    @ReactMethod
    fun pingConfig() {
        CoroutineScope(Dispatchers.Main).launch { dsiEMVManager?.pingConfig() }
    }
    ...
    override fun onCardReadSuccessfully(cardData: CardData) {
        val map = Arguments.createMap().apply { putString("binNumber", cardData.binNumber) }
